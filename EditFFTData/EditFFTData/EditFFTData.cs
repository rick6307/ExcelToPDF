    ...
    public class EditFFTData
    {
        ...
        public static void EditFFT(string inputPath, string outputPath, double maxFreq,  double minFreq,double Amplitude,  string Connstr,int ID_Building,int Type,string logDir,string debugLogFileName,bool IsOutPutFile)
        {
            try
            {
                ...
                int timeoutSeconds = 300; //監看資料夾時間 300 秒 
                List<string> files = new List<string>();
                string[] extensions = new[] { ".csv", ".xlsx"};
                DateTime startTime = DateTime.Now;
                int fileProcessedCount = 0; // 計算處理的檔案數量
                while (fileProcessedCount < 50) {
                    files.Clear();
                    ...
                    foreach(string file in files)
                    {
                        try
                        {
                            if (fileProcessedCount >= 50)
                            {
                                Log.log(debugLogFileName, $"{DateTime.Now.ToString("yyyy/MM/dd hh:mm:ss")} Processed 50 files, exiting loop");
                                return;
                            }
                            ...
                            DateTime dateTimeValue = DateTime.ParseExact(dateTimeString, "yyyy-MM-ddTHH-mm-ss-fff", CultureInfo.InvariantCulture);
                            using (FileStream stream = File.Open(file, FileMode.Open, FileAccess.Read))
                                {
                                    ...
                                    //測試:如果分析出來的頻率振幅比周圍平均振福的4倍還要>，代表它就可能建物主要頻率
                                    ...
                                    {
                                        //測試:如果分析出來的頻率振幅比周圍平均振福的4倍還要小，代表它可能不是建物主要頻率，保留歷時檔不刪除
                                        if (fftdata.Natural_Freq <= maxFreq)
                                        {
                            ...
                            fileProcessedCount++; // 增加已處理檔案的計數
                        }
                        catch (Exception ex)
                        {
                            Log.log(debugLogFileName, $"{DateTime.Now.ToString("yyyy/MM/dd hh:mm:ss")} Error processing file {file}: {ex.Message}");
                        }
                    }
                    // 檢查全局超時
                    if ((DateTime.Now - startTime).TotalSeconds > timeoutSeconds)
                    {
                        Log.log(debugLogFileName, $"{DateTime.Now.ToString("yyyy/MM/dd hh:mm:ss")} Timeout reached, exiting loop");
                        break;
                    }
                }
            }
